...
<C>
The goal is to mock the behavior of the `some_result()` method in the `Foo` class under different conditions. The method takes a ``boolean`` which is `previous_status` which we might not control, and returns a `Result`. Depending on the value of `previous_status`, the method should return either `OK` or `Err`. 
</C>
...
/>
<C>
We'll also need `patch()` to patch the object at hand.
</C>
<Code
  code=
