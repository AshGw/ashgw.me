...
<C>
 ...
 arguments we do not specify if args or kwargs, it retruns anything,
 this anything is not bound to any type, not very type safe if you ask me.
 <S3/>
I'll use a generic `T` as the return type of the function `f` 
``f`` takes an arbitrary number of arguments and keyword arguments
typed as `_PSpec` , this allows us to capture the structure of arguments
 and keyword arguments.  where if you split it you get the ``args`` and ``kwargs``
</C>
<Code
  code=
...
<C>
  Ok but what if the foo function 
...
Between the wrapper and the return function you can use the params as you like,
 ...
 ok but what if your function requires more parameters ? 
