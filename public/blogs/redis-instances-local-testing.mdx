...
<H2>Solution</H2>
<C>
  The whole idea is: you need to have some 
   global singleton object or  a settings loading class that loads configs or whatever 
   you deem fit for your app to determine 
   if you're running in dev or prod whatever you do, just have it switch between
   local development redis connections and prod ones, for local redis you'd need Docker
   , no one runs redis servers directly anyway
   
  <S3/>
  I'll be using Python for this demo with <L href="https://fastapi.tiangolo.com/">FastAPI</L> as a 
  framework,  
