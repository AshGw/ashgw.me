...
<C>
Fetch Metadata headers, introduced last year to provide additional context about the resource request. But all you have to know for now is you can use it  to to check the origin as such:
</C>
<Code
...
For session cookies, ensure they are protected by:
 ...
 - \-  Using the `HttpOnly` attribute to prevent client-side scripts from accessing cookies, thus enhancing security against XSS attacks.
